...
export function useHabitData(onHabitChange?: () => void) {
  ...
  // Improved refresh function with better error handling and debouncing
  const refreshData = useCallback(async (showLoading = true, forceRefresh = false) => {
    // Prevent concurrent refreshes with queueing
  ...
  // Handle visibility changes by refreshing data - with added check to avoid refresh spam
